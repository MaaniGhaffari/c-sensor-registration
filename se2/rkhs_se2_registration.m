classdef rkhs_se2_registration < handle
    % RKHS_SE2_REGISTRATION class
    ...
    properties
        ...
        sigma = 1;            % kernel signal variance (set as std)
    ...
    methods
        ...
        function compute_step_size(obj)
            ...
            xi2_z = (omega_hat^2 * (obj.cloud_y(:,:)') + omega_hat * obj.v)';
            xi3_z = (omega_hat^3 * (obj.cloud_y(:,:)') + omega_hat^2 * obj.v)';
            xi4_z = (omega_hat^4 * (obj.cloud_y(:,:)') + omega_hat^3 * obj.v)';
            
            % The coefficient information
            normxiz2 = vecnorm(xi_z,2,2).^2;
            dotxizxixi = 2 * sum(-xi_z .* xi2_z,2);
            epsil_const = vecnorm(xi2_z,2,2).^2 + 2 * sum(-xi_z .* -xi3_z,2);
            ...
            for i = 1:obj.length_x
                ...
                gamma_i = -temp_coef * (normxiz2(obj.ids{i}) + 2 * sum(xi2_z(obj.ids{i},:) .* y_i,2));
                delta_i = temp_coef * (dotxizxixi(obj.ids{i},:) + 2 * sum(-xi3_z(obj.ids{i},:) .* y_i,2));
                epsil_i = -temp_coef * (epsil_const(obj.ids{i},:) + 2 * sum(xi4_z(obj.ids{i},:) .* y_i,2));
